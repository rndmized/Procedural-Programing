...
void addElement_AtPosition(struct listelement** head_ptr,struct listelement** tail_ptr);
...
void main()
{
	...
	do
	{
    	...
    	if(choice==1){ addElement_AtEnd(&bottom, &top); }
   	 else if (choice == 2) {
    	...
    	else if(choice==3)
    	{
        	...
        	addElement_AtPosition(&top,&bottom);
...
void addElement_AtPosition(struct listelement** head_ptr,struct listelement** tail_ptr)
{
    ...
    // If the user enters a number more than the length of the list refuse the fn
    ...
    // If the user enters the first position call add element at start
    else if (position == 1){
   	 addElement_AtStart(head_ptr,tail_ptr);
    }
    // Add the element at the location
    else
    {
   		 temp = *head_ptr;
   		 ...
   		 //Create the new node
   		 newNode = (struct listelement*)malloc(sizeof(struct listelement));
   		 //Populate the new node's value
   		 printf("Enter integer value to add: ");
   		 scanf("%d", &newNode->value);
   		 //Link up the next node and the previous one with the new node
   		 newNode->previous = temp;
